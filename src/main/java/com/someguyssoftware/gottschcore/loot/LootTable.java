...
public class LootTable {
	   ...
	   private LootTable(LootParameterSet lootParameterSet, LootPool[] lootPools, ILootFunction[] lootFunction) {
	      this.parameterSet = lootParameterSet;
	      this.pools = Lists.newArrayList(lootPools);
	      this.functions = lootFunction;
	      this.combinedFunctions = LootFunctionManager.combine(lootFunction);
	   }
	   ...
	   public List<ItemStack> generate(LootContext context) {
	      ...
	      // TODO remove ForgeHooks - don't need - just adds mod added effects
//	      list = net.minecraftforge.common.ForgeHooks.modifyLoot(list, context);
	   ...
	   public void func_227506_a_(ValidationTracker validationTracker) {
	      for(int i = 0; i < this.pools.size(); ++i) {
	         this.pools.get(i).func_227505_a_(validationTracker.func_227534_b_(".pools[" + i + "]"));
	      }

	      for(int j = 0; j < this.functions.length; ++j) {
	         this.functions[j].func_225580_a_(validationTracker.func_227534_b_(".functions[" + j + "]"));
	      }

	   }

	   public void fillInventory(IInventory inventory, LootContext lootContext) {
	      List<ItemStack> list = this.generate(lootContext);
	      Random random = lootContext.getRandom();
	      List<Integer> list1 = this.getEmptySlotsRandomized(inventory, random);
